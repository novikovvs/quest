...
function createDocument() {
    ...
    buttons.innerHTML = `
		<div class="button" id="startButton">Начать игру</div>
    ...
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            if (blocksArray[i * size + j + 1] != null) {
                ...
                block.id = i * size + j + 1 + '';
                imgId = blocksArray[i * size + j + 1].value;
                block.style.background = `url(../img/${imgId}.jpg)`
...
function pauseStart(newMode) {
    ...
    if (mode === 'pause') {
        ...
        document.getElementById('pauseButton').innerText = 'Продолжить';
